...
export default function ShareCard({ id, seed }: Props) {
  ...
  switch (c) {
    ...
    case "pull": {
      const dare = pick(["Your turn. 30 seconds.", "Bet you can't spot the fake.", "You'd fall for it too.", "Think you'd catch it?", "Make yours before they make you."], id, seed, 5);
      return (
        <div style={{ aspectRatio: "9 / 16", width: "100%", borderRadius: 18, padding: 3, background: "linear-gradient(135deg,#f7d77a,#b8860b 40%,#fff6d0 55%,#b8860b 70%,#f7d77a)", boxShadow: "0 0 26px rgba(247,200,80,0.4)" }}>
          <Frame bg="#0c0a06" border="none">
            ...
            <div style={{ fontWeight: 800, fontSize: 19, color: "#fff", letterSpacing: "-0.02em", lineHeight: 1.0 }}>{hl[0]} {hl[1]}</div>
            {/* the dare — deepfake challenge, not rarity gambling */}
            <div style={{ fontSize: 13, fontWeight: 700, color: "#f7d77a", margin: "6px 0 9px" }}>{dare}</div>
            {/* MAKE YOURS seal — QR is the entry point to deepfake yourself */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(247,215,122,0.08)", border: "1px solid rgba(247,215,122,0.3)", borderRadius: 10, padding: "8px 10px" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 900, color: "#f7d77a", letterSpacing: "0.03em" }}>MAKE YOURS → 30s</div>
