...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Business.IpCopyright" });
  return {
...
export default async function IpCopyrightPage({
  params,
}: {
  params: Promise<{ locale: Locale }>;
}) {
  ...
  const { source } = await getMdxContent("ip-copyright", locale, "business");
  ...
  return (
    <SiteShell>
      ...
      <div className="mt-12 max-w-4xl mx-auto">
        <div className="relative w-full overflow-hidden rounded-lg border border-white/20 bg-zinc-900/5">
      ...
      <div className="mt-10 w-full max-w-6xl mx-auto px-8 md:px-12 lg:px-14">
        <div className="flex justify-end text-sm">
          <Link href="/business/legal-compliance" className="text-white/80 hover:text-white/90">
